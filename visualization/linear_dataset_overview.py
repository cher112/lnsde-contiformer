#!/usr/bin/env python3
"""
生成LINEAR数据集概览图，模仿MACHO数据集概览图的样式
"""
...
def load_linear_data():
    ...
    if data_path is None:
        raise FileNotFoundError("找不到LINEAR数据集文件")
...
def create_linear_overview():
    ...
    try:
        ...
        # 根据LINEAR天体类型创建模拟数据
        class_names = ['RRL', 'EB', 'Ceph', 'LPV', 'Delta_Scuti'] 
        class_counts = [450, 320, 180, 280, 220]  # 模拟的样本数量
        ...
        for i, label in enumerate(all_labels):
            if label == 0:  # RRL - 规则周期变化
                ...
                period = np.random.uniform(0.02, 0.3)
                phase = np.linspace(0, 2*np.pi*period*200, seq_len)
                all_X[i, :, 1] = 13 + 0.1*np.sin(phase) + 0.05*np.random.randn(seq_len)
    ...
    # 主标题
    fig.suptitle('LINEAR Folded Dataset Overview', fontsize=22, fontweight='bold', y=0.95)
...
Class Details:"""
    ...
    print(f"✅ LINEAR数据集概览图已保存: {output_path}")
    
    plt.show()


if __name__ == "__main__":
    create_linear_overview()