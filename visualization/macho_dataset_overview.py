...
def load_macho_data():
    ...
    if data_path is None:
        raise FileNotFoundError("找不到MACHO数据集文件")
...
def create_macho_overview():
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        # 使用原MACHO数据作为参考 (严重不均衡)
        print("使用模拟MACHO数据...")
        
        # 根据原始MACHO数据的严重不均衡分布
        class_names = ['Be', 'CEPH', 'EB', 'LPV', 'MOA', 'QSO', 'RRL']
        class_counts = [128, 101, 255, 365, 582, 59, 610]  # 严重不均衡的分布
        ...
        for i, label in enumerate(all_labels):
            if label == 6:  # RRL - 规则周期变化
                ...
                all_X[i, :, 1] = 14 + 0.8*np.sin(phase) + 0.2*np.random.randn(seq_len)
            elif label == 0:  # Be
                # 间歇性爆发
                all_X[i, :, 1] = 15 + 0.1*np.random.randn(seq_len)
                burst_points = np.random.choice(seq_len, size=int(seq_len*0.1), replace=False)
                all_X[i, burst_points, 1] -= np.random.exponential(1.0, len(burst_points))
            else:  # QSO
                # 随机变化
                all_X[i, :, 1] = 16 + 0.5*np.random.randn(seq_len)
    ...
    # 主标题
    fig.suptitle('MACHO Folded Dataset Overview', fontsize=22, fontweight='bold', y=0.95)
...
Class Details:"""
    ...
    print(f"✅ MACHO数据集概览图已保存: {output_path}")
    
    plt.show()


if __name__ == "__main__":
    create_macho_overview()